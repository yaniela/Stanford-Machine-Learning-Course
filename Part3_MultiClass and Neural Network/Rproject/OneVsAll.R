oneVsAll <- function(X, y, num_labels, lambda) {
  ...
  for (i in 1:num_labels)
  {
   ...
   # digit names (y==i) will return a vector with values 0 or 1. 1 at places where y==i
   #optim  will consider all training data having label c (1-10 note 
   # 0 is mapped to 10) and find the optimal theta vector for it (Classifying white pixels with gray pixels). 
