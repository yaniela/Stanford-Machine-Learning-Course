...
# Because the error in both sets train and cross validation are high
...
fN <- featureNormalize(X_poly, 0, 0)  #Normalize
X_poly <- fN$X_norm
mu <- fN$mu
sigma <- fN$sigma
rm(fN)
...
fNtest <- featureNormalize(X_poly_test, mu, sigma)
X_poly_test <-  cbind(rep(1,dim(X_poly_test)[1]), fNtest$X_norm)

# Map X_poly_val and normalize (using mu and sigma)
X_poly_val <- polyFeatures(Xval, p)
fNVal <- featureNormalize(X_poly_val, mu, sigma)
X_poly_val <-  cbind(rep(1,dim(X_poly_val)[1]), fNVal$X_norm)
...
#  the polynomial regression model is overfitting the training
# data and will not generalize well in the cross validation set (inconsistente % error).
# One way to combat the overfitting (high-variance) problem is to add
# regularization to the model.

# With  λ = 1 regularized polynomial regression model does not have the high-
...
theta <- trainLinearReg(X_poly, y, lambda=2) #Se entrena el modelo con el train set

h <- X_poly_test %*% theta
error_test <- (1 / (2 * dim(X_poly_test)[1])) * sum((h - ytest) ^ 2)

cat(sprintf('Test error (error <- %f)\n\n',error_test))